...
class AnalogPublisher(Node):
    def __init__(self):
        ...
        self.publisher_ = self.create_publisher(AnalogInputs, 'analog_inputs', 10)
    ...
    def publish_analog_data(self):
        ...
        try:
            ...
            data[0] = self.ai_device.a_in(0, AiInputMode.SINGLE_ENDED, self.ai_range)
            data[1] = self.ai_device.a_in(1, AiInputMode.SINGLE_ENDED, self.ai_range)
            data[2] = self.ai_device.a_in(2, AiInputMode.SINGLE_ENDED, self.ai_range)
            data[3] = self.ai_device.a_in(3, AiInputMode.SINGLE_ENDED, self.ai_range)
            data[4] = self.ai_device.a_in(4, AiInputMode.SINGLE_ENDED, self.ai_range)
            data[5] = self.ai_device.a_in(5, AiInputMode.SINGLE_ENDED, self.ai_range)
            data[6] = self.ai_device.a_in(6, AiInputMode.SINGLE_ENDED, self.ai_range)
            data[7] = self.ai_device.a_in(7, AiInputMode.SINGLE_ENDED, self.ai_range)
            msg.data=data
            self.publisher_.publish(msg)
            self.get_logger().info(
                f'Published AI Voltages: [{msg.AI0:.2f}, {msg.AI1:.2f}, {msg.AI2:.2f}, '
                f'{msg.AI3:.2f}, {msg.AI4:.2f}, {msg.AI5:.2f}, {msg.AI6:.2f}, {msg.AI7:.2f}]'
            )
